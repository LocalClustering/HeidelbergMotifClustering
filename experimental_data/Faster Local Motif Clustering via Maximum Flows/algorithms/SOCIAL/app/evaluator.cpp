...
int main(int argn, char **argv) {
	...
	double current_imbalance;
        ...
        } else if(partition_config.seed_node > 0) {
		community.clear();
		community.push_back(partition_config.seed_node-1);
        } else {
                std::cout <<  "Please specify an input partition using the --input_partition flag or a seed node using the flag --seed_node."  << std::endl;
                exit(0);
        }
	...
	std::vector<bool> touched(G.number_of_nodes(), false);
        ...
        G_temp = bfs.runBFS_evaluator(G, community, 0, mapping, subgraph_map, lastLayerNodes,
	...
	long long num_triangles = triangle.triangle_run_graph(*G_temp, lastLayerNodes); // set edge weight to number of triangles that edge belongs to
        ...
        if(partition_config.just_count_triangles) {
	...
	G_temp->set_partition_count(index); 
        ...
        /* boundary.build(); */
        /* boundary.getUnderlyingQuotientGraph(S); */
	S.setNodeWeight(1,1);
