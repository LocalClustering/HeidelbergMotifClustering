...
void graph_from_metis_file_light(graph_access &G, const std::string &filename) {
  MappedFile mapped_file = mmap_file_from_disk(filename);
  const GraphHeader header = read_graph_header(mapped_file);
  G.start_construction_light(header.number_of_nodes, 2 * header.number_of_edges);
