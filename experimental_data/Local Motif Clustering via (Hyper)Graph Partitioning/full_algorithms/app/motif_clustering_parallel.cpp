...
int main(int argn, char **argv)
{
	...
	//hypergraph setup
	// Initialize thread pool with input number of threads and NUMA allocation policy INTERLEAVED
	mt_kahypar_initialize_thread_pool(no_threads, true);
        // Load context from file
	mt_kahypar_context_t* context = mt_kahypar_context_new();
	mt_kahypar_configure_context_from_file(context, "./extern/mt-kahypar/config/default_preset.ini");
	...
	bool was_over_100 = false;
        std::vector<mt_kahypar_partition_id_t> opt_partition; //stores optimal partition
        ...
        std::vector<bool> lastLayerNodes; //true at index [i] if node i is in the last layer
        graph_access *G_temp;
	...
	// Vectors for BFS
		...
		if (hypergraph_node_count > 100) was_over_100 = true;
                ...
                std::vector<mt_kahypar_hyperedge_id_t> eind;
                std::vector<mt_kahypar_hyperedge_weight_t> contracted_hyperedge_weights;
		...
		if (hypergraph_node_count <= 1 || triangle_size == 0) {
			...
			std::cout << "Enumerated Motifs: " << triangle_size << std::endl;
                ...
                //reading hypergraph
                mt_kahypar_hypernode_id_t num_vertices = hypergraph_node_count;
                mt_kahypar_hyperedge_id_t num_hyperedges = contracted_hyperedge_weights.size();
                std::cout << "Nets in Model: " << num_hyperedges << std::endl;             
                
                //run mt-kahypar with chosen imbalance values
                
                for(int beta = 0; beta < partition_config.beta && (beta==0 || t.elapsed() < partition_config.repetition_timelimit); beta++) {
                        
                        const mt_kahypar_partition_id_t k = 2; //partition into two sets
                        ...
                        mt_kahypar_partition(num_vertices, num_hyperedges,
                                current_imbalance, k, 0,
                                /*vertex_weights */ nullptr, const_cast<const mt_kahypar_hyperedge_weight_t*>(&(contracted_hyperedge_weights[0])),
                                const_cast<const size_t*>(&(eptr[0])), const_cast<const mt_kahypar_hyperedge_id_t*>(&(eind[0])),
