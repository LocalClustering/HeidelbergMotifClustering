...
class complete_boundary {
        public:
                ...
                inline void fastComputeQuotientGraphRemoveZeroEdges( graph_access & Q_bar, const NodeID & no_of_blocks);
...
inline void complete_boundary::fastComputeQuotientGraphRemoveZeroEdges( graph_access & Q_bar, const NodeID & no_of_blocks) {
         ...
         std::vector<bool> target_added(no_of_blocks,false);
		...
		block_list_nodes[source_partition].push_back(node);
		 ...
		 for( auto& node : block_list_nodes[source_partition] ) {
			forall_out_edges((*m_graph_ref), e, node) {
				...
				bool is_cut_edge             = (source_partition != target_partition);
				if(is_cut_edge) {
					if (!target_added[target_partition] && target_edgeweight[target_partition] + m_graph_ref->getEdgeWeight(e) > 0) {
						list_targets.push_back(target_partition);
						target_added[target_partition] = true;
					}
		 ...
		 for( auto& target_partition : list_targets ) {
			 ...
			 target_added[target_partition] = false;
		 }
		 list_targets.clear();
         }
