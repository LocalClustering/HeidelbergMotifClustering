/******************************************************************************
 * kway_motif_cond_refinement_core.cpp 
 * *
 * XXX.
 * XXX
 *****************************************************************************/
...
#include "kway_motif_cond_refinement_core.h"
#include "kway_stop_rule_motif_cond.h"
#include "quality_metrics.h"
#include "random_functions.h"

kway_motif_cond_refinement_core::kway_motif_cond_refinement_core() : commons (NULL){
}

kway_motif_cond_refinement_core::~kway_motif_cond_refinement_core() {
        if( commons != NULL)  delete commons;
}
EdgeWeight kway_motif_cond_refinement_core::single_kway_refinement_round(PartitionConfig & config, 
                                                                    graph_access & G, 
...
EdgeWeight kway_motif_cond_refinement_core::single_kway_refinement_round_internal(PartitionConfig & config, 
                                                                    graph_access & G, 
                                                                    complete_boundary & boundary, 
                                                                    boundary_starting_nodes & start_nodes, 
                                                                    int step_limit,
        ...
        if( commons == NULL ) commons = new kway_motif_cond_refinement_commons(config);
        ...
        kway_stop_rule_motif_cond* stopping_rule = NULL;
        switch(config.kway_stop_rule) {
                case KWAY_SIMPLE_STOP_RULE: 
                        stopping_rule = new kway_simple_stop_rule_motif_cond(config);
                        break;
                case KWAY_ADAPTIVE_STOP_RULE: 
                        stopping_rule = new kway_adaptive_stop_rule_motif_cond(config);
                        break;
...
void kway_motif_cond_refinement_core::init_queue_with_boundary(const PartitionConfig & config,
                graph_access & G,
...
void kway_motif_cond_refinement_core::move_node_back(PartitionConfig & config, 
                graph_access & G, 
                NodeID & node,
                PartitionID & to, 
                vertex_moved_hashtable & moved_idx, 
