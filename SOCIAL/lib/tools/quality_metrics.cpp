...
double quality_metrics::local_conductance(graph_access & G, PartitionID larger_block, long long triangle_count) {
        long long edgeCut = 0;
	long long volume = 0;
        forall_nodes(G, n) { 
                ...
                forall_out_edges(G, e, n) {
                        ...
                        if (partitionIDSource != partitionIDTarget) edgeCut += (long long) G.getEdgeWeight(e);
			if (partitionIDSource != larger_block) volume += (long long) G.getEdgeWeight(e);
                } endfor 
        } endfor
        edgeCut = edgeCut/2;
	if (triangle_count != UNDEFINED_TRIANGLE_COUNT) volume = MIN(volume,6*triangle_count-volume); // 6*triangle_count is the summed degree of the graph model
	//std::cout << "Cut: " << edgeCut << " and Vol: " << (double)volume << std::endl;
        double conductance = edgeCut/(double)volume;
        return conductance;
}
