...
int graph_io::readCommunity(graph_access & G, const std::string & filename, std::vector<NodeID> & community, PartitionID block) {
        std::string line;
	...
	community.clear();
		...
		NodeID node = (NodeID) strtoll(line.c_str(), NULL, 10);
		node--; // input range starts at index 1
		community.push_back(node);
                G.setPartitionIndex(node, block);
	}
        /* G.set_partition_count(block+1); */
        in.close();
        return 0;
}
...
int graph_io::readGraphWeighted(graph_access & G, const std::string & filename) {
        ...
        if( 2*nmbEdges > std::numeric_limits<EdgeID>::max() || nmbNodes > std::numeric_limits<NodeID>::max()) {
...
void graph_io::writeCommunity(std::vector<NodeID> & best_community, const std::string & filename) {
        std::ofstream f(filename.c_str());
        std::cout << "writing community to " << filename << " ... " << std::endl;
	for (const auto & node: best_community) f << node+1 <<  "\n";
        f.close();
}
