...
int parse_parameters(int argn, char **argv, 
        ...
        struct arg_str *seed_node                            = arg_str0(NULL, "seed_node", NULL, "Specify seed node (between 1 and n). Default: 1."); //motif clustering
        struct arg_lit *write_cluster                        = arg_lit0(NULL, "write_cluster", "Default: Do not write cluster to disk."); //motif clustering
        struct arg_int *no_threads                           = arg_int0(NULL, "no_threads", NULL, "Specify number of threads for parallel partitioner (mt-kahypar). Default: 1."); //motif clustering
        struct arg_int *beta	                             = arg_int0(NULL, "beta", NULL, "Specify the parameter beta fot local motif clustering. Default: 3."); //motif clustering
        struct arg_lit *keep_parallel_nets                   = arg_lit0(NULL, "keep_parallel_nets", "Default: Retain parallel nets."); //motif clustering
        struct arg_lit *fix_nodes                            = arg_lit0(NULL, "fix_nodes", "Default: Don't fix blocks of seed node and contracted node before partitioning."); //motif clustering
        struct arg_lit *print_cut                            = arg_lit0(NULL, "print_cut", "Default: Don't print cut values."); //motif clustering
        struct arg_lit *mqi_improvement                      = arg_lit0(NULL, "mqi_improvement", "Default: Don't conduct MQI improvement."); //motif clustering
        struct arg_lit *mqi_improvement_star                 = arg_lit0(NULL, "mqi_improvement_star", "Default: Don't conduct MQI improvement with Star Expansion."); //motif clustering
        struct arg_lit *mqi_improvement_clique               = arg_lit0(NULL, "mqi_improvement_clique", "Default: Don't conduct MQI improvement with Clique Expansion."); //motif clustering
        struct arg_lit *mqi_improvement_lawler               = arg_lit0(NULL, "mqi_improvement_lawler", "Default: Don't conduct MQI improvement with Lawler Expansion."); //motif clustering
        struct arg_lit *fix_seed_node                        = arg_lit0(NULL, "fix_seed_node", "Default: Seed node not fixed in MQI."); //motif clustering
        struct arg_str *triangle_count		             = arg_str0(NULL, "triangle_count", NULL, "Specify total triangles count in graph for computing exact motif conductance. Default: not specified.");
        struct arg_lit *label_prop_ls                        = arg_lit0(NULL, "label_prop_ls", "Improve motif conductance with label propagation local search. Default: disabled."); //motif clustering
        struct arg_lit *fm_ls	                             = arg_lit0(NULL, "fm_ls", "Improve motif conductance with FM local search. Default: disabled."); //motif clustering
        struct arg_int *repetition_timelimit                 = arg_int0(NULL, "repetition_timelimit", NULL, "Specify total time limit (in s) for clustering. Default: 3600."); //motif clustering
        struct arg_lit *just_count_triangles                 = arg_lit0(NULL, "just_count_triangles", "Just print the amount of triangles that touch the given community or the given seed node. Default: Disabled."); //motif clustering
        ...
        struct arg_str *bfsdepth_parameter_string            = arg_str0(NULL, "bfsdepth_parameter_string", NULL, "Specify as 3:4:5 or more if you would like to explore further depths.");
		...
		triangle_count,
                seed_node,
		just_count_triangles,
#elif defined MODE_TRIANGLE_COUNTER
                /* k, */   
                /* preconfiguration, */ 
                /* input_partition, */
		/* triangle_count, */
		...
		balance_edges,
                ...
                #endif
                ilp_timeout,
#elif defined MODE_MOTIF_CLUSTERING
                seed_node,
                bfsdepth_parameter_string,
		beta,
                no_threads,
                keep_parallel_nets,
                fix_nodes,
		...
		print_cut,
                mqi_improvement,
                mqi_improvement_star,
                mqi_improvement_clique,
                mqi_improvement_lawler,
                fix_seed_node,
                filename_output, 
		write_cluster,
		repetition_timelimit,
#elif defined MODE_MOTIF_CLUSTERING_GRAPH
                seed_node,
                bfsdepth_parameter_string,
		triangle_count,
                filename_output, 
        ...
        if(bfsdepth_parameter_string->count) {
                std::istringstream f(bfsdepth_parameter_string->sval[0]);
                std::string s;    
                partition_config.bfsDepths.clear();
                while (getline(f, s, ':')) {
                        partition_config.bfsDepths.push_back(stoi(s));
                }       
        } 
        ...
        if(filename_output->count > 0) {
                ...
                partition_config.write_cluster = true;
        }
	...
	if(mh_enable_tabu_search->count > 0) {
        ...
        if (seed_node->count > 0) {
                partition_config.seed_node = strtoll(seed_node->sval[0], NULL, 10);
        }

        if (beta->count > 0) {
                partition_config.beta = beta->ival[0];
        }

        if (no_threads->count > 0) {
                partition_config.no_threads = no_threads->ival[0];
        }

        if(write_cluster->count > 0) {
                partition_config.write_cluster = true;
        }

        if(keep_parallel_nets->count > 0) {
                partition_config.keep_parallel_nets = true;
        }

        if(fix_nodes->count > 0) {
                partition_config.fix_nodes = true;
        }

        if (triangle_count->count > 0) {
                partition_config.triangle_count = strtoll(triangle_count->sval[0], NULL, 10);
        }

        if(print_cut->count > 0) {
                partition_config.print_cut = true;
        }

        if(mqi_improvement->count > 0) {
                partition_config.mqi_improvement = true;
        }

        if(mqi_improvement_star->count > 0) {
                partition_config.mqi_improvement_star = true;
        }

        if(mqi_improvement_clique->count > 0) {
                partition_config.mqi_improvement_clique = true;
        }

        if(mqi_improvement_lawler->count > 0) {
                partition_config.mqi_improvement_lawler = true;
        }

        if(fix_seed_node->count > 0) {
                partition_config.fix_seed_node = true;
        }

        if(label_prop_ls->count > 0) {
                partition_config.label_prop_ls = true;
        }

        if(fm_ls->count > 0) {
                partition_config.fm_ls = true;
        }

        if (repetition_timelimit->count > 0) {
                partition_config.repetition_timelimit = repetition_timelimit->ival[0];
        }

        if(just_count_triangles->count > 0) {
                partition_config.just_count_triangles = true;
        }
